...
import { canAuditTarget, privilegedSupabaseClient, requireAdminPasswordResetCaller, requireTargetAccountId, targetRole } from '../../utils/adminPasswordReset';
...
export default defineEventHandler(async (event) => {
  ...
  for (const accountId of accountIds) {
    if (canAuditTarget(caller.id, caller.role, accountId, await targetRole(privileged, accountId))) eligibleAccountIds.push(accountId);
  }
