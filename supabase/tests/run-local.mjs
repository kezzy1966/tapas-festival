...
try {
  ...
  for (const name of (await readdir(migrationsDir)).filter(n => /^(20260911000[123]00_festival_.*|20260924000100_festival_venues|20260925000100_festival_year)\.sql$/.test(n)).sort()) {
  ...
  let total = 1;
  for (const name of (await readdir(testsDir)).filter(n => /^(festival_integrity|festival_ratings|festival_security|festival_venues|festival_year)\.sql$/.test(n)).sort()) {
