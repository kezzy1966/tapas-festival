...
try {
  ...
  await db.exec(`
    ...
    create table storage.objects (id uuid primary key default gen_random_uuid(), bucket_id text not null, metadata jsonb default '{}'::jsonb);
  ...
  for (const name of (await readdir(migrationsDir)).filter(n => /^[0-9]{14}_.+\.sql$/.test(n)).sort()) {
