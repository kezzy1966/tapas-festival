-- Development-only deterministic test data for Castellón Tapas Festival 2025.
-- Creates/fetches user1@example.test through user100@example.test, then replaces
-- only those users' ratings for this one festival. It never touches other users.
...
select
  ...
  'email',
  now(), now(), now()
from generate_series(1, 100) as fixture(number)
...
do $$
begin
  if (select count(*) from seed_accounts) <> 100 then
    raise exception 'expected 100 deterministic test accounts';
  end if;
end $$;
...
-- Reruns replace only fixture ratings associated with this festival, preserving
-- all manual/real ratings and fixture activity in other festivals.
...
insert into festival.reviews (tapa_id, user_id, rating, review_text, moderation_status)
select tapa_id, user_id, rating, review_text, 'visible'
...
select establishment_id, user_id, rating
