...
const blankEstablishment = () => ({ festival_id: '', name: '', description_en: '', description_es: '', address: '', coordinates: '', phone: '', whatsapp: '', facebook_url: '', website_url: '', hours_notes_en: '', hours_notes_es: '', is_published: false, participation_status: 'active', closure_status: 'normal' });
...
function editEstablishment(row: any) { editingEstablishment.value = row; establishmentForm.value = { ...blankEstablishment(), ...row, coordinates: row.latitude == null || row.longitude == null ? '' : `${row.latitude}, ${row.longitude}` }; tab.value = 'establishments'; }
...
async function saveEstablishment() {
  ...
  const { coordinates: _coordinates, ...formValues } = f;
