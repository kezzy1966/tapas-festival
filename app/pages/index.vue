...
const abbreviatedIdentity = computed(() => {
  ...
  if (!domain) return local.length > 4 ? `${local.slice(0, 2)}…${local.slice(-1)}` : local;
  const suffix = domain.includes('.') ? domain.slice(domain.lastIndexOf('.')) : domain.slice(-3);
  return `${local.slice(0, Math.min(3, local.length))}…@…${suffix}`;
});
...
async function rate(tapa: Tapa, rating: number) {
  ...
  if (draft.length > maxReviewLength) { ratingBusy.value = null; ratingError.value = t('reviewTooLong'); return; }
