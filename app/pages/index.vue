...
type TrendingTapa = { tapa_id: string; new_rating_count: number };
...
const { emergencyShutdown } = usePublicSiteControls();
const { now: openingHoursNow, openingHoursStatus } = useOpeningHoursStatus();
const openNowOnly = ref(false);
function establishmentHoursStatus(venue: Establishment) { return openingHoursStatus(venue.opening_hours, openingHoursNow.value); }
function establishmentHoursLabel(venue: Establishment) { const status = establishmentHoursStatus(venue); return status === 'open' ? 'Open now' : status === 'closed' ? 'Closed now' : 'Hours unavailable'; }
const { theme, setTheme } = useFestivalTheme();
const loading = ref(true);
const ratingBusy = ref<string | null>(null);
const error = ref('');
const ratingError = ref('');
const festival = ref<Festival | null>(null);
const festivalControls = ref({ festival_active: true, ratings_enabled: true, reviews_enabled: true, rankings_enabled: true, total_rating_counts_enabled: true, want_to_try_enabled: true, public_read_only: false });
const festivalControlsAvailable = ref(false);
const establishments = ref<Establishment[]>([]);
const tapas = ref<Tapa[]>([]);
const stats = ref<Record<string, Stats>>({});
const trendingTapas = ref<Record<string, TrendingTapa>>({});
...
function toggleWantedTapa(tapaId: string) {
  if (!canUseWantToTry.value) return;
...
async function refreshAdminAccess() {
  if (!user.value) { isAdminAccount.value = false; return; }
  const { data, error: roleError } = await db().rpc('is_current_admin');
...
const establishmentFor = (tapa: Tapa) => establishments.value.find((venue) => venue.id === tapa.establishment_id);
function canParticipateAtEstablishment(venue: Establishment) { return isAdminAccount.value || venue.participation_status === 'active'; }
function canParticipateInTapa(tapa: Tapa) { const venue = establishmentFor(tapa); return Boolean(venue && tapa.participation_status === 'active' && canParticipateAtEstablishment(venue)); }
...
const showTotalRatingCount = computed(() => festivalControlsAvailable.value ? festivalControls.value.total_rating_counts_enabled : festival.value?.show_total_rating_count !== false);
const rankingsEnabled = computed(() => festivalControlsAvailable.value ? festivalControls.value.rankings_enabled : festival.value?.show_rankings !== false);
const festivalActive = computed(() => !festivalControlsAvailable.value || festivalControls.value.festival_active);
const ratingsEnabled = computed(() => !festivalControlsAvailable.value || festivalControls.value.ratings_enabled);
const reviewsEnabled = computed(() => !festivalControlsAvailable.value || festivalControls.value.reviews_enabled);
const wantToTryEnabled = computed(() => !festivalControlsAvailable.value || festivalControls.value.want_to_try_enabled);
const publicReadOnly = computed(() => festivalControlsAvailable.value && festivalControls.value.public_read_only);
const canUseRatings = computed(() => isAdminAccount.value || (!userSuspended.value && festivalActive.value && ratingsEnabled.value && !publicReadOnly.value));
const canUseReviews = computed(() => isAdminAccount.value || (!userSuspended.value && festivalActive.value && reviewsEnabled.value && !publicReadOnly.value));
const canUseWantToTry = computed(() => !userSuspended.value && festivalActive.value && wantToTryEnabled.value && !publicReadOnly.value);
const totalTapaRatings = computed(() => Object.values(stats.value).reduce((total, item) => total + Number(item.rating_count || 0), 0));
type MobilePaneLayout = 'normal' | 'rankings-expanded' | 'bars-expanded' | 'rankings-interaction' | 'bars-interaction';
type ExpandablePane = 'rankings' | 'bars';
const mobilePaneLayout = ref<MobilePaneLayout>('normal');
function toggleMobilePaneLayout(target: 'rankings-expanded' | 'bars-expanded') {
  mobilePaneLayout.value = mobilePaneLayout.value === target ? 'normal' : target;
}
function expandSmallPaneForInteraction(pane: ExpandablePane) {
  if (!import.meta.client || !window.matchMedia('(max-width: 639px)').matches) return;
  const paneIsSmall = pane === 'rankings'
    ? mobilePaneLayout.value === 'bars-expanded' || mobilePaneLayout.value === 'bars-interaction'
    : mobilePaneLayout.value === 'rankings-expanded' || mobilePaneLayout.value === 'rankings-interaction';
  if (paneIsSmall) mobilePaneLayout.value = pane === 'rankings' ? 'rankings-interaction' : 'bars-interaction';
}
...
const sortedEstablishments = computed(() => establishments.value.filter((venue) => !openNowOnly.value || establishmentHoursStatus(venue) === 'open').slice().sort((a, b) => {
...
async function revealEstablishment(establishmentId: string) {
  selectedEstablishmentId.value = establishmentId;
...
async function loadStats() {
  ...
  const { data, error: statsError } = await db().rpc('public_tapa_rating_stats', { p_festival_id: festival.value.id });
  if (statsError) { error.value = statsError.message; return; }
  stats.value = Object.fromEntries((data || []).map((item: Stats) => [item.tapa_id, item]));
}
async function loadTrendingTapas() {
  trendingTapas.value = {};
  if (!festival.value) return;
  const { data, error: trendingError } = await db().from('tapa_recent_rating_activity').select('tapa_id,new_rating_count').eq('festival_id', festival.value.id);
  if (trendingError) { error.value = trendingError.message; return; }
  trendingTapas.value = Object.fromEntries((data || []).map((item: TrendingTapa) => [item.tapa_id, item]));
}
...
let establishmentRefreshPromise: Promise<boolean> | null = null;
async function refreshFestivalControls() {
  if (!festival.value) return;
  if (controlsRefreshPromise) return controlsRefreshPromise;
  controlsRefreshPromise = (async () => {
    const { data: controls, error: controlsError } = await db().from('festival_controls').select('festival_active,ratings_enabled,reviews_enabled,rankings_enabled,total_rating_counts_enabled,want_to_try_enabled,public_read_only').eq('festival_id', festival.value!.id).maybeSingle();
    if (!controlsError && controls) { festivalControls.value = controls; festivalControlsAvailable.value = true; }
    else { festivalControlsAvailable.value = false; }
  })();
  try { await controlsRefreshPromise; } finally { controlsRefreshPromise = null; }
}
async function refreshFestivalEstablishments(): Promise<boolean> {
  if (!festival.value) return false;
  if (establishmentRefreshPromise) return establishmentRefreshPromise;
  establishmentRefreshPromise = (async () => {
    ...
    if (venueError) { error.value = venueError.message; return false; }
    establishments.value = venueRows || [];
    return true;
  })();
  try { return await establishmentRefreshPromise; } finally { establishmentRefreshPromise = null; }
}
function refreshFestivalDataOnFocus() { void Promise.all([refreshFestivalControls(), refreshFestivalEstablishments(), refreshUserSuspension()]); }
function refreshFestivalDataOnVisibilityChange() {
  if (document.visibilityState === 'visible') void Promise.all([refreshFestivalControls(), refreshFestivalEstablishments(), refreshUserSuspension()]);
}
async function loadFestival() {
  ...
  await refreshFestivalControls();
  if (!await refreshFestivalEstablishments()) { loading.value = false; return; }
  ...
  loadWantedTapas();
  await Promise.all([loadStats(), loadTrendingTapas(), loadEstablishmentStats(), loadMyReviews(), loadMyEstablishmentReviews(), loadPublicReviews()]);
  loading.value = false;
}
async function rate(tapa: Tapa, rating: number) {
  if (!canUseRatings.value || !canParticipateInTapa(tapa)) { ratingError.value = publicReadOnly.value ? 'Public site is currently read-only.' : !canParticipateInTapa(tapa) ? t('withdrawn') : 'Ratings are temporarily disabled.'; return; }
  ...
  await Promise.all([loadStats(), loadTrendingTapas(), loadEstablishmentStats(), loadMyEstablishmentReviews(), loadPublicReviews()]);
}
async function rateEstablishment(venue: Establishment, rating: number) {
  if (!canUseRatings.value || !canParticipateAtEstablishment(venue)) { ratingError.value = publicReadOnly.value ? 'Public site is currently read-only.' : !canParticipateAtEstablishment(venue) ? t('withdrawn') : 'Ratings are temporarily disabled.'; return; }
...
async function removeEstablishmentRating(venue: Establishment) {
  if (publicReadOnly.value) { ratingError.value = 'Public site is currently read-only.'; return; }
  const existing = myEstablishmentReviews.value[venue.id];
...
async function saveReview(tapa: Tapa): Promise<boolean> {
  if (!canUseReviews.value || !canParticipateInTapa(tapa)) { ratingError.value = publicReadOnly.value ? 'Public site is currently read-only.' : !canParticipateInTapa(tapa) ? t('withdrawn') : 'Reviews are temporarily disabled.'; return false; }
  const existing = myReviews.value[tapa.id];
...
const canSaveUnsavedReview = computed(() => {
  const tapa = tapas.value.find((item) => item.id === unsavedReviewTapaId.value);
  return Boolean(tapa && canUseReviews.value && canParticipateInTapa(tapa));
});
...
function openPasswordReset() { void navigateTo({ path: '/reset-password', query: authEmail.value.trim() ? { email: authEmail.value.trim() } : undefined }); }
...
watch(user, () => {
  ...
  void Promise.all([loadMyReviews(), loadMyEstablishmentReviews(), refreshAdminAccess(), refreshUserSuspension()]);
}, { immediate: true });
onMounted(() => {
  window.addEventListener('beforeunload', onBeforeUnload);
  window.addEventListener('focus', refreshFestivalDataOnFocus);
  document.addEventListener('visibilitychange', refreshFestivalDataOnVisibilityChange);
  void loadFestival();
});
onUnmounted(() => {
  window.removeEventListener('beforeunload', onBeforeUnload);
  window.removeEventListener('focus', refreshFestivalDataOnFocus);
  document.removeEventListener('visibilitychange', refreshFestivalDataOnVisibilityChange);
});
</script>
...
<template>
  <main id="main-content" class="min-h-screen bg-stone-50 text-stone-900" tabindex="-1">
    <section class="mx-auto max-w-6xl pb-8 sm:px-5 sm:pt-4 md:px-8"><p v-if="loading" class="text-stone-600">{{ t('loading') }}</p><p v-else-if="error" class="rounded-lg border border-red-200 bg-red-50 p-4 text-red-800">{{ t('unableToLoad') }} {{ error }}</p><p v-else-if="!festival" class="rounded-lg border border-stone-200 bg-white p-5 text-stone-600">{{ t('noFestival') }}</p>
      <template v-else>
        <p v-if="isAdminAccount && emergencyShutdown" class="mx-4 mb-4 rounded border-2 border-red-600 bg-red-50 p-3 text-center text-sm font-extrabold text-red-800 sm:mx-0">PUBLIC SITE IS CURRENTLY DISABLED</p>
        <p v-if="!festivalActive" class="mx-4 mb-4 rounded border border-amber-300 bg-amber-50 p-3 text-center text-sm font-semibold text-amber-900 sm:mx-0">{{ language === 'es' ? 'El festival no está actualmente activo.' : 'This festival is not currently active.' }}</p>
        <p v-else-if="publicReadOnly && !isAdminAccount" class="mx-4 mb-4 rounded border border-amber-300 bg-amber-50 p-3 text-center text-sm font-semibold text-amber-900 sm:mx-0">{{ language === 'es' ? 'El sitio público está actualmente en modo de solo lectura.' : 'The public site is currently read-only.' }}</p>
        <div class="mx-4 mb-4 flex flex-col gap-2 rounded-lg border border-stone-200 bg-white px-3 py-2 sm:mx-0 sm:mb-6 sm:flex-row sm:items-center sm:justify-between">
          <div class="min-w-0"><p class="text-xs font-semibold uppercase tracking-widest text-emerald-700">Tapas Castellón</p><h1 class="whitespace-nowrap font-display text-[clamp(1rem,5.5vw,1.75rem)] font-bold leading-tight tracking-tight">{{ festival ? text(festival.name_en, festival.name_es) : 'Tapas festival' }}</h1></div>
          <div class="flex w-full shrink-0 flex-wrap items-center gap-2 sm:w-auto"><div class="flex shrink-0 flex-wrap items-center gap-2"><div v-if="user" class="flex items-center gap-2 rounded-md border border-stone-300 bg-stone-50 px-2 py-1 text-xs font-semibold text-stone-700"><span>{{ t('signedIn') }}</span><span v-if="abbreviatedIdentity" :title="t('signedIn')">{{ abbreviatedIdentity }}</span><NuxtLink v-if="isAdminAccount" to="/admin" class="rounded bg-emerald-700 px-2 py-1 text-white shadow-sm">Admin</NuxtLink><button type="button" class="rounded bg-white px-2 py-1 text-emerald-700 shadow-sm ring-1 ring-stone-200" @click="logout">{{ t('logOut') }}</button></div><template v-else><button type="button" class="rounded-md border border-stone-300 px-3 py-2 text-sm font-bold text-emerald-700" @click="openAuth('signin')">{{ t('signIn') }}</button><button type="button" class="rounded-md bg-emerald-700 px-3 py-2 text-sm font-bold text-white" @click="openAuth('signup')">{{ t('createAccount') }}</button></template></div><div class="flex basis-full items-center justify-center gap-2 sm:basis-auto sm:justify-start"><div class="flex overflow-hidden rounded-md border border-stone-300 text-lg font-bold" :aria-label="t('language')"><button type="button" class="min-h-11 min-w-11 px-3 py-2" :class="language === 'es' ? 'bg-emerald-700 ring-2 ring-inset ring-emerald-950' : 'bg-white text-stone-600'" :aria-pressed="language === 'es'" aria-label="Español" title="Español" @click="setLanguage('es')">🇪🇸</button><button type="button" class="min-h-11 min-w-11 border-l border-stone-300 px-3 py-2" :class="language === 'en' ? 'bg-emerald-700 ring-2 ring-inset ring-emerald-950' : 'bg-white text-stone-600'" :aria-pressed="language === 'en'" aria-label="English" title="English" @click="setLanguage('en')">🇬🇧</button></div><button type="button" class="rounded-md border border-stone-300 px-2 py-2 text-[11px] font-extrabold tracking-wide transition" :class="openNowOnly ? 'bg-emerald-700 text-white' : 'bg-stone-100 text-stone-800'" :aria-pressed="openNowOnly" aria-label="Toggle opening-hours filter" @click="openNowOnly = !openNowOnly">{{ openNowOnly ? "OPEN NOW" : "ALL TIMES" }}</button><div class="flex overflow-hidden rounded-md border border-stone-300 text-lg font-bold" aria-label="Appearance"><button type="button" class="min-h-11 min-w-11 px-3 py-2" :class="theme === &quot;light&quot; ? &quot;bg-amber-100 ring-2 ring-inset ring-amber-500&quot; : &quot;bg-white text-stone-600&quot;" :aria-pressed="theme === &quot;light&quot;" aria-label="Light mode" title="Light mode" @click="setTheme(&quot;light&quot;)">☀️</button><button type="button" class="min-h-11 min-w-11 border-l border-stone-300 px-3 py-2" :class="theme === &quot;dark&quot; ? &quot;bg-slate-700 ring-2 ring-inset ring-slate-950&quot; : &quot;bg-white text-stone-600&quot;" :aria-pressed="theme === &quot;dark&quot;" aria-label="Dark mode" title="Dark mode" @click="setTheme(&quot;dark&quot;)">🌙</button></div></div></div>
        </div>
        <div><ClientOnly><FestivalMap :establishments="establishments" :open-now-only="openNowOnly" :tapas="tapas" :stats="stats" :my-reviews="myReviews" :location-active="true" :selected-establishment-id="selectedEstablishmentId" :wanted-tapas="wantedTapas" :show-rating-counts="showTotalRatingCount" @select-tapa="revealTapaFromMap" @select-establishment="revealEstablishmentFromMap" /></ClientOnly></div>
        ...
        <form v-if="!user && authMode" class="mx-4 mb-4 rounded-lg border border-stone-200 bg-white p-3 sm:mx-0 sm:mb-6" @submit.prevent="submitPublicAuth">
          ...
          <div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]"><input v-model="authEmail" class="min-w-0 rounded border border-stone-300 px-3 py-2 text-sm" type="email" autocomplete="email" :placeholder="t('email')" required><div class="min-w-0"><div class="flex min-w-0 items-center gap-1"><input v-model="authPassword" class="min-w-0 flex-1 rounded border border-stone-300 px-3 py-2 text-sm" :type="authPasswordVisible ? 'text' : 'password'" :autocomplete="authMode === 'signin' ? 'current-password' : 'new-password'" :placeholder="`${t('password')} · ${t('passwordHint')}`" minlength="8" required><button type="button" class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded border border-stone-300 text-stone-600" :aria-label="authPasswordVisible ? 'Hide password' : 'Show password'" :title="authPasswordVisible ? 'Hide password' : 'Show password'" v-on:click="authPasswordVisible = !authPasswordVisible"><Icon :name="authPasswordVisible ? 'lucide:eye-off' : 'lucide:eye'" class="h-4 w-4" aria-hidden="true" /></button></div><button v-if="authMode === 'signin'" type="button" class="text-left text-xs font-semibold text-emerald-700 hover:underline" v-on:click="openPasswordReset">Forgot password?</button></div><button class="rounded bg-emerald-700 px-4 py-2 text-sm font-bold text-white disabled:opacity-50" :disabled="authBusy">{{ authBusy ? (authMode === 'signin' ? t('signingIn') : t('creatingAccount')) : (authMode === 'signin' ? t('signIn') : t('createAccount')) }}</button></div>
        </form>
        ...
        <div class="mx-2 grid items-start gap-2 sm:mx-0 sm:gap-4 lg:gap-8" :class="!(showRankings && rankingsEnabled) ? 'grid-cols-1' : mobilePaneLayout === 'rankings-expanded' ? 'grid-cols-[minmax(0,72fr)_minmax(0,28fr)] sm:grid-cols-[minmax(0,38fr)_minmax(0,62fr)]' : mobilePaneLayout === 'bars-expanded' ? 'grid-cols-[minmax(0,28fr)_minmax(0,72fr)] sm:grid-cols-[minmax(0,38fr)_minmax(0,62fr)]' : mobilePaneLayout === 'rankings-interaction' ? 'grid-cols-[minmax(0,60fr)_minmax(0,40fr)] sm:grid-cols-[minmax(0,38fr)_minmax(0,62fr)]' : mobilePaneLayout === 'bars-interaction' ? 'grid-cols-[minmax(0,40fr)_minmax(0,60fr)] sm:grid-cols-[minmax(0,38fr)_minmax(0,62fr)]' : 'grid-cols-[minmax(0,38fr)_minmax(0,62fr)]'">
          <aside v-if="showRankings && rankingsEnabled" class="sticky top-2 h-[calc(100vh-1rem)] self-start overflow-y-auto pr-1 sm:top-4 sm:h-[calc(100vh-2rem)] sm:pr-2" @click.capture="expandSmallPaneForInteraction('rankings')">
            <section class="rounded-xl border border-stone-200 bg-white p-2 sm:p-5"><div class="flex flex-wrap items-baseline justify-between gap-2"><h2 class="font-display text-lg font-bold sm:text-2xl">{{ t("liveRankings") }}</h2><div class="flex items-center gap-1"><p v-if="showTotalRatingCount" class="text-xs font-semibold text-stone-600">Total tapa ratings: {{ totalTapaRatings.toLocaleString() }}</p><button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded border border-stone-300 bg-white text-lg font-bold text-emerald-700 transition hover:bg-emerald-50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-700 sm:hidden" :aria-label="mobilePaneLayout === 'rankings-expanded' ? 'Restore normal split' : 'Expand rankings'" :title="mobilePaneLayout === 'rankings-expanded' ? 'Restore normal split' : 'Expand rankings'" :aria-pressed="mobilePaneLayout === 'rankings-expanded'" @click="toggleMobilePaneLayout('rankings-expanded')">→</button></div></div><ol class="mt-3 grid gap-2 sm:mt-4"><li v-for="(tapa, index) in rankedTapas" :key="tapa.id"><button type="button" class="block w-full rounded-lg border border-transparent bg-stone-50 px-3 py-2 text-left text-sm transition hover:bg-emerald-50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-700" :class="{ trending: trendingTapas[tapa.id] }" @click="revealTapaFromRanking(tapa)"><p v-if="trendingTapas[tapa.id]" class="mb-1 text-[10px] font-extrabold tracking-widest text-red-700">TRENDING</p><p class="truncate font-semibold"><strong>{{ index + 1 }}.</strong> {{ text(tapa.name_en, tapa.name_es) }}</p><p class="truncate text-xs text-stone-500">{{ establishmentNameFor(tapa) }}</p><p class="mt-1 text-xs font-semibold text-stone-700">Rating: {{ summaryFor(tapa.id).average_rating == null ? "—" : Number(summaryFor(tapa.id).average_rating).toFixed(1) }} ★<template v-if="showTotalRatingCount"> ({{ summaryFor(tapa.id).rating_count }})</template> · Score: {{ credibilityScore(tapa).toFixed(1) }}</p><blockquote v-if="index < 10 && highlightedReviewFor(tapa.id)" class="mt-1 border-l-2 border-amber-400 pl-2 text-xs text-stone-600"><p class="line-clamp-2 italic">“{{ highlightedReviewFor(tapa.id)?.review_text }}”</p></blockquote></button></li></ol></section>
          </aside>
          <div ref="establishmentsPane" class="sticky top-2 h-[calc(100vh-1rem)] overflow-y-auto pr-1 sm:top-4 sm:h-[calc(100vh-2rem)] sm:pr-2" @click.capture="expandSmallPaneForInteraction('bars')">
        <p v-if="!rankingsEnabled" class="mb-3 rounded border border-stone-200 bg-white p-3 text-sm text-stone-600">{{ language === 'es' ? 'Las clasificaciones están temporalmente desactivadas.' : 'Rankings are temporarily disabled.' }}</p>
...
.trending { border-color: #dc2626; border-width: 2px; }

:global(html.dark) { background: #172033; }
:global(html.dark body), :global(html.dark #__nuxt) { background: #172033; color: #e5e7eb; }
:global(html.dark .bg-stone-50) { background-color: #172033 !important; }
:global(html.dark .bg-white) { background-color: #223047 !important; }
:global(html.dark .bg-emerald-50) { background-color: #173b35 !important; }
:global(html.dark .bg-stone-100) { background-color: #2b3a52 !important; }
:global(html.dark .text-stone-900), :global(html.dark .text-stone-800), :global(html.dark .text-stone-700) { color: #f1f5f9 !important; }
:global(html.dark .text-stone-600), :global(html.dark .text-stone-500) { color: #cbd5e1 !important; }
:global(html.dark .border-stone-200), :global(html.dark .border-stone-300) { border-color: #475569 !important; }
:global(html.dark input), :global(html.dark textarea), :global(html.dark select) { background: #1e293b !important; color: #f8fafc !important; border-color: #64748b !important; }
:global(html.dark .leaflet-popup-content-wrapper), :global(html.dark .leaflet-popup-tip), :global(html.dark .leaflet-control) { background: #223047; color: #f1f5f9; }
:global(html.dark .leaflet-popup-content a), :global(html.dark .leaflet-control a) { color: #d1fae5; }
:global(html.dark .trending) { border-color: #fb7185 !important; background-color: #3a2230 !important; }
</style>
