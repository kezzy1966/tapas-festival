...
async function rateEstablishment(venue: Establishment, rating: number) {
  ...
  const result = existing
    ? await db().from('establishment_reviews').update({ rating }).eq('id', existing.id)
    : await db().from('establishment_reviews').insert({ establishment_id: venue.id, user_id: currentUserId.value, rating });
      ...
      <template v-else>
        ...
        <div class="mx-2 grid items-start gap-2 sm:mx-0 sm:gap-4 lg:gap-8" :class="showRankings ? 'grid-cols-[minmax(0,38fr)_minmax(0,62fr)]' : 'grid-cols-1'">
          <aside v-if="showRankings" class="sticky top-2 h-[calc(100vh-1rem)] self-start overflow-y-auto pr-1 sm:top-4 sm:h-[calc(100vh-2rem)] sm:pr-2">
            <section class="rounded-xl border border-stone-200 bg-white p-2 sm:p-5"><h2 class="font-display text-lg font-bold sm:text-2xl">{{ t('liveRankings') }}</h2><p class="mt-1 text-xs text-stone-600 sm:text-sm">{{ t('rankingDescription') }}</p><ol class="mt-3 grid gap-2 sm:mt-4"><li v-for="(tapa, index) in rankedTapas" :key="tapa.id"><button type="button" class="grid w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-x-2 rounded-lg bg-stone-50 px-2 py-2 text-left text-xs transition hover:bg-emerald-50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-700 sm:px-3 sm:text-sm" @click="revealTapaFromRanking(tapa)"><div class="min-w-0"><p class="truncate font-semibold"><strong>{{ index + 1 }}.</strong> {{ text(tapa.name_en, tapa.name_es) }}</p><p :ref="(element) => setRankingTicker(tapa.id, element)" class="overflow-hidden whitespace-nowrap text-xs text-stone-500"><span v-if="rankingTickerOverflow[tapa.id]" class="ranking-ticker-track"><span>{{ establishmentNameFor(tapa) }}</span><span aria-hidden="true">{{ establishmentNameFor(tapa) }}</span></span><span v-else class="inline-block whitespace-nowrap">{{ establishmentNameFor(tapa) }}</span></p></div><div class="shrink-0 text-right"><p class="whitespace-nowrap font-semibold">{{ t('publicRating') }} {{ summaryFor(tapa.id).average_rating == null ? '—' : Number(summaryFor(tapa.id).average_rating).toFixed(1) }} ★ ({{ summaryFor(tapa.id).rating_count }})</p><p class="whitespace-nowrap text-xs text-stone-500">{{ t('rankingScore') }} {{ credibilityScore(tapa).toFixed(1) }}<span v-if="summaryFor(tapa.id).good_excellent_percentage != null"> · {{ Number(summaryFor(tapa.id).good_excellent_percentage).toFixed(0) }}% {{ t('goodExcellent') }}</span></p></div><blockquote v-if="index < 10 && highlightedReviewFor(tapa.id)" class="col-span-2 w-full border-l-2 border-amber-400 pl-2 text-xs text-stone-600"><p class="line-clamp-2 italic">“{{ highlightedReviewFor(tapa.id)?.review_text }}”</p></blockquote></button></li></ol></section>
          </aside>
        ...
        <section class="mx-4 mt-6 rounded-xl border border-stone-200 bg-white p-4 text-sm text-stone-600 sm:mx-0">
          ...
          <dl class="mt-2 grid gap-1"><div><dt class="inline font-semibold text-stone-800">{{ t('location') }}</dt><dd class="inline"> {{ festival.city }}</dd></div></dl>
        </section>
        <footer class="mx-4 mt-6 pb-4 text-center sm:mx-0"><button type="button" class="text-xs font-semibold text-emerald-700 underline" @click="reportOpen = true">{{ t('reportProblem') }}</button><p class="mt-2 text-[10px] text-stone-400">{{ buildReference }}</p></footer>
