...
function refreshFestivalControlsOnFocus() { void refreshFestivalControls(); }
function refreshFestivalControlsOnVisibilityChange() {
  if (document.visibilityState === 'visible') void refreshFestivalControls();
}
...
async function rate(tapa: Tapa, rating: number) {
  if (!canUseRatings.value) { ratingError.value = publicReadOnly.value ? 'Public site is currently read-only.' : 'Ratings are temporarily disabled.'; return; }
...
async function saveReview(tapa: Tapa): Promise<boolean> {
  if (!canUseReviews.value) { ratingError.value = publicReadOnly.value ? 'Public site is currently read-only.' : 'Reviews are temporarily disabled.'; return false; }
  const existing = myReviews.value[tapa.id];
...
onMounted(() => {
  ...
  window.addEventListener('focus', refreshFestivalControlsOnFocus);
  document.addEventListener('visibilitychange', refreshFestivalControlsOnVisibilityChange);
  void loadFestival();
});
onUnmounted(() => {
  window.removeEventListener('beforeunload', onBeforeUnload);
  window.removeEventListener('focus', refreshFestivalControlsOnFocus);
  document.removeEventListener('visibilitychange', refreshFestivalControlsOnVisibilityChange);
});
</script>
