...
const canUseRatings = computed(() => isAdminAccount.value || (festivalActive.value && ratingsEnabled.value && !publicReadOnly.value));
const canUseReviews = computed(() => isAdminAccount.value || (festivalActive.value && reviewsEnabled.value && !publicReadOnly.value));
const canUseWantToTry = computed(() => festivalActive.value && wantToTryEnabled.value && !publicReadOnly.value);
...
function refreshFestivalDataOnFocus() { void Promise.all([refreshFestivalControls(), refreshFestivalEstablishments()]); }
function refreshFestivalDataOnVisibilityChange() {
  if (document.visibilityState === 'visible') void Promise.all([refreshFestivalControls(), refreshFestivalEstablishments()]);
}
...
watch(user, () => {
  ...
  void Promise.all([loadMyReviews(), loadMyEstablishmentReviews(), refreshAdminAccess()]);
}, { immediate: true });
onMounted(() => {
